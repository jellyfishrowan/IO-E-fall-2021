...
document.getElementById("title").innerHTML = "Sprint 1";
document.getElementById("details").innerHTML = "working with ML5";
...
{// ---------- Define Actions and dependancies ----------
    ...
    setup = () => {
        ...
        // cameraInput.size(windowWidth, windowWidth * 0.5625);
        // cameraAspectRatio = cameraInput.width / cameraInput.height;
        // console.log("W = "+cameraInput.width+"; H = "+cameraInput.height);
        // console.log("Aspect ratio: "+cameraAspectRatio);
        // cameraInput.size(windowWidth, windowHeight);
...
{// ---------- Define Main Loop(s) ----------
    draw = () => {
        ...
        //     // console.log(bodyPoses[0].pose.keypoints);
        //     for(let i = 0; i < bodyPoses[0].pose.keypoints.length; i++){
        //         // console.log(bodyPoses[0].pose.keypoints[i]);
        //         // bodyPoses[0].pose.keypoints[i].part;
        //         // bodyPoses[0].pose.keypoints[i].position.x;
        //         // bodyPoses[0].pose.keypoints[i].position.y;
        //         // bodyPoses[0].pose.keypoints[i].score;
        //         if(bodyPoses[0].pose.keypoints[i].score > 0.2){
        //             // let xPosition = bodyPoses[0].pose.keypoints[i].position.x;
        //             // let yPosition = bodyPoses[0].pose.keypoints[i].position.y;
        //             ellipse(bodyPoses[0].pose.keypoints[i].position.x, bodyPoses[0].pose.keypoints[i].position.y, windowWidth / 100);
        //             // console.log("X="+bodyPoses[0].pose.keypoints[i].position.x+"; Y="+bodyPoses[0].pose.keypoints[i].position.y);
        //         }
        //     }
        //     console.log(bodyPoses[0].pose);
        // }
